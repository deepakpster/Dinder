...
export default class Stories extends Component {
  componentWillMount(){
    ...
    const userStories = firebase.database().ref('user-stories/' + uid)
        ...
        <Content>
          {
            Object.keys(stories).map((key)=>{
              console.log('Stories', stories[key])
              return (
                <Card key={key}>
                  <CardItem>
                    <Left>
                  ...
                  </CardItem>
                  {
                    (stories[key].photoURL.length>0) ?
                      ...
                      {stories[key].photoURL.map((photo, ii, arr)=>{
                        return(<Image key={ii} style={{width:width/arr.length-1, height: 100}} source={{uri:photo}}/>)
                      })
                      }
                      </CardItem>
                    : <Spinner/>
                  ...
                  <CardItem>
                    <Left>
                      <Button transparent danger rounded onPress={() => alert('Trash')}>
                        <Icon active name="trash" />
                      </Button>
                    </Left>
                    <Right>
                      <Text>{Moment(stories[key].createdOn).fromNow()}</Text>
                    </Right>
                  </CardItem>
                </Card>
              )
